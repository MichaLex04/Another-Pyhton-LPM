...
x=datos[["sepal length (cm)","sepal width (cm)"]]
...
print("Cantidad de flores por cluster:")
for cluster, cantidad in sorted(clusters_count.items()):
    print(f"Cluster {cluster}: {cantidad} flores")

plt.scatter(x["sepal length (cm)"],x["sepal width (cm)"],c=etiquetas)
